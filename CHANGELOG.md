...
ConsistenCy 2.0 - 现代代码健康智能守护者正式发布！
...
#### 🔄 一致性检测
- **命名风格检测** - snake_case / camelCase / PascalCase 一致性检查
- **函数签名分析** - 类型注解风格一致性
- **异常处理风格** - try/except vs error values 模式检测
- **导入风格分析** - import x vs from x import y
- **Embedding 支持** - sentence-transformers 语义相似度计算

#### 🔥 技术债务热点
- **圈复杂度分析** - 基于 radon 或启发式算法
- **变更频率统计** - 基于 git log 的历史分析
- **热点计算** - 复杂度 × 频率的归一化分数
- **风险分级** - critical/high/medium/low 四级风险

#### 🤖 AI 审查
- **LiteLLM 集成** - 支持 DeepSeek、Claude、Grok 等任意模型
- **Prompt 模板系统** - 5 种审查类型（通用/安全/一致性/性能/文档）
...
- **文件行级评论** - 对特定代码行发表评论
- **批量评论** - 并发发布多条评论
- **PR 标签** - 自动添加 passed/issues-found 标签

#### 📈 Dashboard
- **Streamlit 界面** - 6 个标签页（概览/安全/热点/漂移/AI/趋势）
- **交互式图表** - Plotly 饼图、散点图、柱状图、折线图
- **历史趋势** - 30 天问题数量和扫描耗时趋势
- **数据持久化** - 自动保存扫描历史

#### 🖥️ CLI
- **完整命令集** - analyze / ci / dashboard / scan / config / init
- **Rich 输出** - 漂亮的表格、进度条、彩色文本
- **配置验证** - 检查 LLM/GitHub/GitNexus 配置状态

#### 🔧 CI/CD
- **GitHub Actions** - PR 触发、uv 缓存、多任务并行
- **Docker 支持** - 多阶段构建，production/dev/dashboard 三阶段
- **Docker Compose** - 支持 CLI、Dashboard、测试等多种模式
